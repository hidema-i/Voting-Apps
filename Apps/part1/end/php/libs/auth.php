...
class Auth
{
  ...
  public static function login($id, $pwd)
  {
    try {
      ...
      Msg::push(Msg::ERROR, 'ログイン処理でエラーが発生しました。少し時間を置いてから再度お試し下さい。');
    }
  ...
  public static function regist($user)
  {
    try {
      ...
      $is_success = false;
      Msg::push(Msg::DEBUG, $e->getMessage());
      Msg::push(Msg::ERROR, 'ユーザー登録でエラーが発生しました。少し時間をおいてください。');
    }
  ...
  public static function requirePermission($topic_id, $user)
  {
    if (!static::hasPermission($topic_id, $user)) {
      Msg::push(Msg::ERROR, '編集権限がありません。ログインしてから再度試してみて下さい。');
      redirect('login');
    }
  }
}
